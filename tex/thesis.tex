...
The NEAT algoritm was first described in 2002 by Kenneth O. Stanley and Risto Miikkulainen in their paper \cite{originalNeat}.
Core idea behind it is to use an evolutionary algorithm to evolve a neural network. This idea has been tried before (for example, in TWEANNs),
but NEAT is innovating on this by introducing a few key ideas. One of them is the concept of starting from a minimal structure (as opposed to random strucure in TWEANNs)
for each problem. 
That means, that every solution starts from a single input layer connected densely to the output layer. Creating any additional structure is up to the
algorithm. Another key idea is the concept of innovation numbers. This one comes from the observation, that in a system that can evolve both
topology and weights, a lot of the time system can represent similar solutions with a widely different structure and encoding. This makes it harder for further
combining working parts of different genomes. This is solved by having a global innovation number for the whole population,
and assigning it to each new conneection. This way, we can tell when two genomes separately evolved the same connection, thereby making it easier for them to
combine. Last key idea is the concept of speciation. This is a way to make sure that the population is diverse,
and that the algorithm is not stuck in a local minimum. This works by dividing the population into species by calculating how different (in both structure and weights)
each genome is form the randomly selected representative of a specie. The abstraction of a specie is useful because they represent a group of genomes that are 
trying out a similar solution to the problem, and a lot of the configuration metaparameters focus on the behavior of species. To make them relevant,
NEAT algorithm evaluates fitness of each genome only in the relative context of its specie (This is called adjusted fitness, because it's adjusted to the context of its specie).
This means that it's unlikely that one specie will dominate the whole population and end up in a local minimum.
\section{Methodology}
    \subsection{Problems used to test the implementation}
        ...
        This problem is also used to test if the algorithm can generate minimal structure - the original paper \cite{originalNeat}
...
Xor 1 av. generations 124.94, std 63.4, av\_nodes 9.48 avg\_cons 8.22 
Xor 2 av. generations 136.31, std 70.5, av\_nodes 3.5, av\_cons 23.37

Upon closer inspection, the results of default xor run and the xor run with a 'bias' node are the reverse of what they should be.
The bias node should make the problem easier, but it seems to make it harder.
