...
\usepackage{titling}
\usepackage{tabularx}
\title{Minimalistic implementation of NeuroEvolution of Augmenting Topologies}
...
\begin{titlepage}
    \centering
    \vskip 60pt
    \LARGE Minimalistic implementation of NeuroEvolution of Augmenting Topologies \par
    \vskip 3em
    \large Przemysław Pilipczuk \par
    \vskip 1.5em
    December 2023
    \vskip 6em
    ...
    \raggedright
\section*{Abstract}
In this work I have implemented a fully functional  NeuroEvolution of Augmenting Topologies (NEAT) algorithm 
with a focus on simplicity and readability. The implementation aims to be a good resource for learning and teaching about NEAT.
The generic core algorithm is ~400 lines of Python code, and to adapt it to a specific problem requires about ~70 
more. The terseness of source code makes it a good starting point for anyone interested in understanding NEAT, 
...
\section*{Abstrakt}
W tej pracy została zaimplementowana w pełni funkcjonalna wersja algorytmu NeuroEvolution of Augmenting Topologies (NEAT)
z naciskiem na prostotę i czytalność. Implementacja ma na celu być dobrym źródłem nauki i nauczania o NEAT.
Rdzeń algorytmu zajmuje około 400 linii kodu w języku Python, a do dostosowania go do konkretnego problemu potrzeba około kolejnych
70. Zwięzłość kodu źródłowego sprawia, że jest to dobry punkt wyjścia dla każdego, kto chce poznać NEAT.
Implementacja została przetestowana na 4 problemach: XOR, Cartpole, Acrobot i BipedalWalker.

\section*{Słowa kluczowe}
Neuroewolucja, NEAT, Algorytmy ewolucyjne, Uczenie maszynowe, Sztuczna inteligencja, Dydaktyczna implementacja 
  \end{titlepage}
...
During 1990s and early 2000s the ideas of evolving neural networks, and doing it in a way that allows for structural changes gained more traction. 
Notably, the TWEANNs (Topology and Weight Evolving Artificial Neural Network) algorithms were the first realization of this concept.
...
This process is inspired by biological evolution, where both the structure and function of biological neural networks
...
\par However, the early TWEANN algorithms had a couple of problems \cite{originalNeat}. 
First one of them was the problem of competing conventions \cite{Schaffer_Whitley_Eshelman}. It arises when two different genomes encode the same
...
Lastly, the TWEANN algorithms of that time did not have a mechanism to maintain diversity in the population,
...
Direct encoding means that the genotype is directly translated into phenotype (such as a neural network) \cite{Angeline_Saunders_Pollack_1994}. This is the case with a NEAT algorithm.
...
Indirect encoding, on the other hand offers a more abstract and compact representation \cite{Reeves}. Instead of mapping directly each component to the solution,
...
\par Tree-based encoding, often used in genetic programming, represents solutions as tree structures, 
...
The effectiveness of a genetic algorithm is heavily dependent on the appropriateness of its encoding scheme to the problem at hand, as it directly affects the algorithm's ability to generate and evolve valid, high-quality solutions \cite{HyperNeat}.
    \begin{figure}[h]
        \centering
        \includegraphics[width=0.8\textwidth]{encoding.png}
        \caption{Encoding of a neural network in NEAT \\ Source: \cite{originalNeat}}
    \end{figure}
...
The learning in such network is usually done by using backpropagation to adjust weights. There is also an error function, sometimes called loss function, that measures how far the given output of the network is from the wanted one.
...
will lead to the global maximum. It also requires the loss function to be differentiable, which is not always the case. Later, alternative approaches
...
for a computation that has to be done in the worst case $N*M*S$ times every run, where $N$ is the number of generations, $M$ is the number of genomes in a population
and $S$ is the amount of species).
Encoding information in such a way also helps to prevent the competing conventions problem. This arises when two different genomes encode the same
...
Another key idea of NEAT is the minimal starting structure. Unlike previous TWEANN approaches that began with random and potentially complex structures \cite{originalNeat}, 
...
that in case of a percentage of population dying out the algorithm can start from scratch by inserting new genomes with minimal structure, and it will be forced to grow again. 
...
the offspring count is proportional to how well a specie is doing. One of the most common and universal functions for that purpuse is the following: 
...
of the matching genes.
    \begin{figure}[h]
        ...
        \caption{Showcase of crossover process \\ Source: \cite{originalNeat}}
    \end{figure}
...
the structure is often referred as 'exploration', as the algorithm is exploring the wider solution space of topologies for a given problem, while the
process of using the structure while changing weights is often referred as 'exploitation', as the algorithm is exploiting the current solution.
...
\begin{figure}[H]
    ...
    \centering
      ...
      \caption{Genome after mutation}
    \end{subfigure}
    \caption{Genome before and after adding a connection}
  \end{figure}
...
Then, the generation loop starts. During each generation, the algorithm does the following:
    ...
    \subsection{Why control theory problems?}
    Out of the four problems three can be categorized as control theory problems, so it might be beneficial to explain why is this type of problem in general
...
\begin{table}[H]
    ...
    \end{table}
        ...
        \subsection{Cartpole}
        Cartpole is a problem taken from Farama Foundation's Gymnasium library \cite{gymnasium}. It is a classic control theory problem,
    ...
    \section{Implementation details}
        ...
        \begin{itemize}
            \item dill - used for serializing and deserializing particular genomes both for inspection and debugging purposes. 
        ...
        \end{itemize}
        Additionally, some of the problems require additional dependencies. Those being:
        \begin{itemize}
        ...
        The core algorithm is implemented in ~400 lines of Python, in model.py
    ...
    % Zastępujące wygenerowaną brzydką tabelę taką z dividerem
...
\begin{table}[H]
    ...
    \caption{Overview of how the offspring calculation is affected by negative values in its inputs.}
    \end{table}
    ...
    I have implemented a couple of minimalistic tools that can be used to inspect parts of
...
def after_finished(p:Population):
  ...
  with open(f"{p.path}/champStats.txt", "w") as f:
    f.write(champStats)
\end{verbatim}
    \par I could easily extend this mechanism to save more data, such as the number of nodes
    and connections in the best genome, make it run on every iteration with different data,
    ...
    \par To conclude the matters of introspection, we have implemented a couple of
...
\begin{itemize}
\item XOR
\par XOR problem was tested on multiple configurations, allowing us to present the results in a table below
...
generations being 4.45, standard deviation being 3.5. I also measured mean and standard deviation
...
generations being 2.77, standard deviation being 1.91. I also measured mean and standard deviation
...
of exploit its current topology. 
    ...
    The case of bipedalwalker is the most interesting one. The model can reliably learn to walk to the end of the map (corresponding
